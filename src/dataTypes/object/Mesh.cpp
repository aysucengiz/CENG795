    ...
    else
    {
        ...
        while (verticesStream >> vert[0] >> vert[1] >> vert[2])
        {
            ...
            if (vert[0] != vert[1] && vert[0] != vert[2] && vert[1] != vert[2])
            {
                ...
                globalBbox.vMax.y = std::max(maxv.y, globalBbox.vMax.y);
                globalBbox.vMax.z = std::max(maxv.z, globalBbox.vMax.z);
                globalBbox.vMin.x = std::min(minv.x, globalBbox.vMin.x);
                globalBbox.vMin.y = std::min(minv.y, globalBbox.vMin.y);
                globalBbox.vMin.z = std::min(minv.z, globalBbox.vMin.z);
...
void Mesh::getBitan(const Vertex& v, Vec3r& pT, Vec3r& pB, int triID, bool normalize, real time) const
{
    return  Faces[triID]->getBitan(v,pT,pB, triID, normalize, time);
}
