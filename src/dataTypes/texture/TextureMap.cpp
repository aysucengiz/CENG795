...
{
    MipMap m0;
    if (!contains(filename, ".hdr") && !contains(filename, ".exr"))
    {
        unsigned char* data = PPM::read_image_ldr(filename.c_str(), m0.width, m0.height, channels_in_file, 3);
    ...
    {
        float *data = nullptr;
        if (contains(filename, ".hdr"))
            data = PPM::read_image_hdr(filename.c_str(), m0.width, m0.height, channels_in_file, 3);
        else
            data = PPM::read_image_exr(filename.c_str(), m0.width, m0.height, channels_in_file, 3);
...
/// IMAGE TEXTURE ////
ImageTexture::ImageTexture(uint32_t id, DecalMode d, Image* image, Interpolation interp,
                           real normalizer) : Texture(id, d), image(image), normalizer(normalizer)
{
...
{
    Texel xy = {
        (real)fmod(tex.u, 1.0) * (image->mipmaps[level].width), (real)fmod(tex.v, 1.0) * (image->mipmaps[level].height)
    };
...
Color PerlinTexture::TextureColor(const Vertex& vert, Texel& tex, real level) const
{
    real result = 0.0;
...
Color CheckerTexture::TextureColor(const Vertex& vert, Texel& tex, real level) const
{
